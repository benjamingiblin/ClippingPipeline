...
if [ "$Check_Sys" == "IA" ]; then
    # This should have Mask set to NoMask FYI.
...
#for i in `seq 0 24`; do   
	...
	for k in `seq $j 4`; do  # $j 4
		...
		for RR in `seq $RR_start $RR_end`; do 
			...
			# one noise real. per LOS - NON-MOSAIC!
			#f=${TCdir}/MRres${MRres}_${Sys_Tag}100Sqdeg_SN${SN[$j]}_${Mask}_${Survey}GpAM_z${Survey}_${ZBlabel}_Cosmol${i}/ThBins9/SN${SN[$j]}_${filetag}.${Survey}GpAM.LOS${los}.${file_type}
		    fi
		    ...
		    paramfile2=$param_dir/${Sys_Tag}100Sqdeg_SN${SN[$k]}_${Mask}_${Survey}GpAM_X3sigma_SS${SS}_z${Survey}_ZBcut${ZBcut[$k]}_ThBins9_Cosmol${i}_MRres${MRres}ec
			...
			if [ "$Launch_Specific_LOS" == "True" ]; then
			    ...
			    echo $f
			    echo ${Sys_Tag} $i ${SN[$j]} ${ZBcut[$j]} ${SN[$k]} ${ZBcut[$k]} ${SS} ${los} #R${RR} #${los_end}
			    count=$((count+1))
			    if [ "$j" -eq "$k" ]; then
				# auto-bin
				...
				sbatch Launch_Calc_CrossCorr_Cycle_z_And_LOS.sh $paramfile1 $paramfile2	$los $los $RR $RR
			    fi
