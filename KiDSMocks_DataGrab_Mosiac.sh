...
# The format of the KiDS-1000 mosiac mocks is so different
...
if [[ ${ENDname[-1]} == *"Cosmol"* ]]; then
    ...
    mosiac_DIR=/disk10/jharno/PeakCount/Mocks/Cosmo_KiDS1000_Angus_m/${cosmol_fname}_${seed}/
    input=${mosiac_DIR}/AllLOS/KiDS1000_MocksCat_${cosmol_fname}_${seed}_${bnum}_LOS1-25_R${R}.dat

    # Use the LOS to determinewhich columns to pull out:
    col_g1=$((los_num-1))  # subtract 1 because cosmoSLICS LOS start at 1, not 0.
    ...
    # It's a SLICS mosiac KiDS1000 run...
    # multiple LOS stored in a single cat, find which to copy:
    if [ "$los_num" -lt "199" ]; then
        los_low=74
    elif [ "$los_num" -lt "299" ]; then
        los_low=200
    fi
    mosiac_DIR=/disk10/jharno/PeakCount/Mocks/Cov_KiDS1000/SLICS_Angus
    input=${mosiac_DIR}/KiDS1000_MocksCat_SLICS_${bnum}_LOS${los_low}*_R${R}.dat
...
# (weight is not unity with mosiac mocks)
