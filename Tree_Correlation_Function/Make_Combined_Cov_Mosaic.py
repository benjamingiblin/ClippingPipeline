...
NLOS_flag = 217 #Nreal for 18 region K1000 survey. 3906 #715
...
PDF_SS_label = 'SS2.816' #'SS1.408-2.816-5.631' 
PDF_nbins = 4
PDF_DIR = '/home/bengib/Clipping_Pipeline/Mass_Recon/%s100Sqdeg_%s_%s_%sGpAM_z%s_ZBcut%s/' %(MRres,noise[-1],Mask,
                                                                                            Survey, Survey,
                                                                                            ZBcut[-1])

PDF_fname = PDF_DIR + '%s_%s.%sGpAM.NLOS%s-Shuffle%s.%s.SNRPDF_%sbins.zbins11-12-13-14-15-22-23-24-25-33-34-35-44-45-55.npy' %(noise[-1],
                                                                                                                              ftag,Survey,
                                                                                                                              NLOS_flag,
                                                                                                                              Shuffle_Config,
                                                                                                                              PDF_SS_label,
...
if mock_Type == 'SLICS':
        filenames_c = sorted( glob.glob( '%s/%s_%s.%sGpAM.LOS*R1.SS%s.rCLIP_X3sigma.CorrFun.asc'%(DIRname[0],noise[0],
                                                                                                  ftag,Survey,
                                                                                                  SS[0]) ) )
        Realisations_c = len(filenames_c)
        filenames_uc = sorted( glob.glob( '%s/%s_%s.%sGpAM.LOS*R1.ORIG.CorrFun.asc'%(DIRname[0],noise[0],
                                                                                     ftag, Survey)) )
...
for ss in range(len(SS)):
        for d in range(dir_cycle):
                ...
                for R in range(1,Regions+1):
                        ...
                        filenames_c = sorted( glob.glob( '%s/*_%s.%sGpAM.LOS*R%s.SS%s.rCLIP_X3sigma.CorrFun.asc'%(DIRname[d],
                                                                                                                  ftag,Survey,R,
                                                                                                                  SS[ss])) )
                        filenames_uc = sorted( glob.glob( '%s/*_%s.%sGpAM.LOS*R%s.ORIG.CorrFun.asc'%(DIRname[d],ftag,Survey,R)) )
                        ...
                        for i in range(len(filenames_c)):
                                ...
                                xi_p_c_all[i, index*nbins:(index+1)*nbins] += np.loadtxt( filenames_c[i], usecols=(3,), unpack=True) /Regions
                                xi_m_c_all[i, index*nbins:(index+1)*nbins] += np.loadtxt( filenames_c[i], usecols=(4,), unpack=True) /Regions
                                # dividing by 18 here, so after scrolling through R=18 regions, it will have saved xi+/- avg'd across 18R 
                                
                        for i in range(len(filenames_uc)):
                                print( "Reading in unclipped file %s of %s"%(i,len(filenames_uc)) )
                                xi_p_uc_all[i, index*nbins:(index+1)*nbins] += np.loadtxt( filenames_uc[i], usecols=(3,), unpack=True) /Regions
                                xi_m_uc_all[i, index*nbins:(index+1)*nbins] += np.loadtxt( filenames_uc[i], usecols=(4,), unpack=True) /Regions

                # We have avg'd the xi+/- across 18 regions. Now avg across 217 LOS:
...
if Combine_with_PDF:
        ...
        fname_cov_PDF_uc = fname_cov_uc.replace('UCxUC+-', 'PDF-UCxUC+-', 1)
