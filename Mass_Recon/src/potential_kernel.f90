        subroutine potential_kernel_g1(twoD_DATA_IN,nx,ny)
!        subroutine potential_kernel_g1(twoD_DATA_IN,nx,ny,func)
...
!  smoothing in Fourier space ; f4 is the fourth power wavenumber module
!  the wavenumber is (k1,k2)
        do j=1,n2/2+1
          ...
          f4=(k1*k1+k2*k2)**2
          Wfactor=1.
          if(f4.ne.0.) Wfactor=-(-k1*k1+k2*k2)/f4
          Y_OUT_2D(1,2*j-1)=Y_OUT_2D(1,2*j-1)*Wfactor
          Y_OUT_2D(1,2*j)=Y_OUT_2D(1,2*j)*Wfactor
        enddo
        do i=2,n1/2
          do j=1,n2
            ...
            Wfactor=1.
            if(f4.ne.0.) Wfactor=-(-k1*k1+k2*k2)/f4
            Y_OUT_2D(2*i-1,j)=Y_OUT_2D(2*i-1,j)*Wfactor
            Y_OUT_2D(2*i,j)=Y_OUT_2D(2*i,j)*Wfactor
          enddo
        enddo
        do j=1,n2/2+1
          k1=1./2.
          ...
          Wfactor=1.
          if(f4.ne.0.) Wfactor=-(-k1*k1+k2*k2)/f4
          Y_OUT_2D(n1+1,2*j-1)=Y_OUT_2D(n1+1,2*j-1)*Wfactor
          Y_OUT_2D(n1+1,2*j)=Y_OUT_2D(n1+1,2*j)*Wfactor
        enddo
...
!        Status = DftiSetValue(my_descl_handle, DFTI_BACKWARD_SCALE, Scale)
        ...
        subroutine potential_kernel_g2(twoD_DATA_IN,nx,ny,func)
...
!        include '/home/waerbeke/fitsio/cookbook.f90'
!        include '/home/waerbeke/recipes/bessj.for'
!        include '/home/waerbeke/recipes/bessj0.for'
!        include '/home/waerbeke/recipes/bessj1.for'
