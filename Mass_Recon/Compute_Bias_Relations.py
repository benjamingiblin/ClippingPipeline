...
Mask = 'Mosaic'
SS = 5.631 #1.408           # smoothing scale used in clipped measurements
...
sys_array_store = [] # for dz sys, the nodes are different for each tomo bin, so store them.
k=0
for i in range(len(noise)):
    for j in range(i, len(noise)):
        ...
        for t in range(nthbins):
            ...
            scaler, inpred = Apply_Norm( pred_tmp )
            if Sys=="dz" and i!=j:
                # fit a 2D GP as func of dz1 & dz2:
                GPR_Class = GPR_Emu(sys_array, inpred, np.zeros_like(inpred), sys_array)
                _,_,HPs_perz[t,:] = GPR_Class.GPRsk(np.ones(3), None, 100) # 100&500 give same answer
            else:
                # linear fit
                HPs_perz[t,:], cov = curve_fit(linear(x0=sys_array[Sys_idx0]), sys_array, inpred.flatten())

        # In Make_SNR-PDF, the dz bias was computed relative to dz=-0.89*sigmaz; so bias=0 for this point.
        ...
        if Sys=="dz":
            ...
            for t in range(nthbins):
                ...
                if i==j:
                    ...
                    b0_tmp = linear(x0=sys_array[0])(0., HPs_perz[t,:]) # in mean0,std1 units
                    b0[t] = scaler.inverse_transform( b0_tmp.reshape(-1,1) ).flatten() # in PDF units
                    # we MAY need to re-fit linear, since the b0 correction was computed in trans'd space, but applied in real:
                    scaler, inpred = Apply_Norm( pred_tmp-b0[t] )
                    HPs_perz[t,:], cov = curve_fit(linear(x0=0.), sys_array, inpred.flatten()) # HPs now for b0-corrected PDFs
                    
                else:
                    GPR_Class = GPR_Emu(sys_array, inpred, np.zeros_like(inpred), [[0,0]])
    ...
    for j in range(i, len(noise)):
        ...
        if Sys=="dz" and i!=j:
            ...
            for t in range(nthbins): 
                ...
                scaler, inpred = Apply_Norm( pred_tmp )
                GPR_Class = GPR_Emu(sys_array_store[k], inpred, np.zeros_like(inpred), GP_Sys_vals*[sigma[i],sigma[j]] )
            ...
            for s in range(len(Sys_files_all)):
                if '_X_' not in Sys_files_all[s]: # then dz1=dz2
                    plot_mock[k,:,scount] = pdf_Sys_bias[k][s,:] 
            ...
            for t in range(nthbins):
                pred_tmp = pdf_Sys_bias[k][:,t].reshape(-1,1)
                scaler, inpred = Apply_Norm( pred_tmp )
                outpred = linear(x0=0.)(Sys_vals_p0*sigma[i], HPs[k][t,:])
                plot_fit[k,t,:] = scaler.inverse_transform( outpred.reshape(-1,1) ).flatten()
                plot_mock[k,t,:] = pdf_Sys_bias[k][:,t] 
